...
以上对于 InnerStack 的操作已经完成，在 Pod 详情页你可以在 "Replicas/Service Port Mapping" 项目中看到系统自动给 PostgreSQL 对外的 tcp/5432 端口分配了内网IP和端口，另外你可以在 "Applications/Configs" 里面查看系统自动创建的密码信息，你可以使用 PostgreSQL 客户端访问这个实例确认其有效性。
