...
此文档主要介绍如何在 InnerStack 上构建并部署 MySQL 5.7 服务(单机版)，先分析下 MySQL 部署特征:

MySQL 基于 C,CPP 编程语言开发，InnerStack 默认的 g3, g2 规格容器基于 CentOS-8/rhel-8/CentOS-7/rhel-7, 所以编译 MySQL 也必须兼容这两种系统之一, 此实例我们在 CentOS 8 环境中编译打包。


此实例我们将要构建两个原始包

1. mysql57 为 MySQL 原始安装包 
2. mysql-keeper 为 InnerStack 基于 go 开发的 MySQL 配置工具

注: mysql-keeper 配置工具默认将屏蔽MySQL root 系统管理帐号，而最终提供一个普通权限的帐号和固定数据库名(dbaction)和用户名(dbuser)的访问权限，主要基于两个考虑

1. 安全改进
2. 简化, 在云原生环境中，扩展数据库的方式是新建App/Pod, 而不是在一个数据库系统中建立多个数据库或帐号
...
``` shell
# CentOS 8
yum install -y gzip-devel bison litirpc rpcgen libtirpc-devel
...
PUSH /opt/src/inpack/mysql57/mysql57-5.7.25-1.el8.x64.txz
  ok mysql57 100%
```
...
> TODO: 后续提供 MySQL Group Replication 分布式集群 AppSpec 方案文档
